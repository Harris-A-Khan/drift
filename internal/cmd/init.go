...
func runInit(cmd *cobra.Command, args []string) error {
	...
	poolerHost, poolerPort, branchPoolerHosts := resolveDatabasePoolerConfig(supabaseProjectRef)
	configContent := generateConfig(name, pType, teamID, bundleID, supabaseProjectRef, supabaseProjectName, poolerHost, poolerPort, branchPoolerHosts)
...
func resolveDatabasePoolerConfig(projectRef string) (string, int, map[string]string) {
	host := config.DefaultDatabasePoolerHost
	port := config.DefaultDatabasePoolerPort
	branchHosts := map[string]string{}

	if projectRef == "" {
		return host, port, branchHosts
	}

	client := supabase.NewClientWithRef(projectRef)
	...
	connInfo, err := client.GetBranchConnectionInfo(branchName)
	if err == nil && connInfo != nil {
		if connInfo.PoolerHost != "" {
			host = connInfo.PoolerHost
		}
		if connInfo.PoolerPort != 0 {
			port = connInfo.PoolerPort
		}
	}

	for _, b := range branches {
		lookupBranch := b.GitBranch
		if lookupBranch == "" {
			lookupBranch = b.Name
		}
		if lookupBranch == "" || lookupBranch == branchName {
			continue
		}

		branchConnInfo, err := client.GetBranchConnectionInfo(lookupBranch)
		if err != nil || branchConnInfo == nil || branchConnInfo.PoolerHost == "" {
			continue
		}

		if branchConnInfo.PoolerHost != host {
			branchHosts[lookupBranch] = branchConnInfo.PoolerHost
		}
	}
...
func formatBranchPoolerHosts(branchPoolerHosts map[string]string) string {
	if len(branchPoolerHosts) == 0 {
		return ""
	}

	keys := make([]string, 0, len(branchPoolerHosts))
	for branch, host := range branchPoolerHosts {
	...
	section.WriteString("  branch_pooler_hosts:\n")
	for _, branch := range keys {
		section.WriteString(fmt.Sprintf("    %q: %s\n", branch, strings.TrimSpace(branchPoolerHosts[branch])))
	}

	return section.String()
}

func generateConfig(name, projectType, teamID, bundleID, supabaseRef, supabaseName, poolerHost string, poolerPort int, branchPoolerHosts map[string]string) string {
...
`, functionsDir, migrationsDir)
	...
	configContent := fmt.Sprintf(`# .drift.yaml - Project configuration for drift CLI
...
`
		...
		if teamID != "" || bundleID != "" {
			configContent += fmt.Sprintf(`apple:
  team_id: "%s"
  bundle_id: "%s"
...
`
		...
		configContent += fmt.Sprintf(`xcode:
  xcconfig_output: %s
...
`, xcconfigPath, schemesConfig)
	...
	// Common sections
	branchPoolerHostsSection := formatBranchPoolerHosts(branchPoolerHosts)
...
#     skip_secrets:
#       - ENABLE_DEBUG_SWITCH
`, poolerHost, branchPoolerHostsSection, poolerPort, config.DefaultDatabaseDirectPort, wtPattern)
